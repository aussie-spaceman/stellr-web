...
export default async function CheckInPage({
  ...
  return (
    <div className="min-h-screen bg-surface flex items-start justify-center px-4 py-10">
      <div className="w-full max-w-md bg-white rounded-2xl border border-line shadow-sm p-6 space-y-5">
        <div className="text-center">
          <p className="text-xs font-semibold text-indigo-600 uppercase tracking-wide">
            {isVirtual ? 'Confirm Your Attendance' : 'Event Check-In'}
          </p>
          <h1 className="text-xl font-bold text-ink mt-1">{event.title}</h1>
          {event.date && <p className="text-sm text-content-muted mt-0.5">{formatDate(event.date)}</p>}
        </div>
        {token ? (
