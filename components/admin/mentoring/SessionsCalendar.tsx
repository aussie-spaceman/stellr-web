...
import type { CalendarSession } from '@/lib/mentoring'

const WEEKDAYS = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun']
// Stable palette assigned per cohort so each cohort's chips share a colour.
const PALETTE = ['#7C5CFC', '#3C6DF6', '#1FA97A', '#E0922F', '#16B6C4', '#D9433C', '#5B3FE0', '#0F6A4C']

export function SessionsCalendar({ sessions }: { sessions: CalendarSession[] }) {
  const today = useMemo(() => new Date(), [])
  const [cursor, setCursor] = useState({ y: today.getFullYear(), m: today.getMonth() })

  const colorByCohort = useMemo(() => {
    const map = new Map<string, string>()
    let i = 0
    for (const s of sessions) if (!map.has(s.cohortId)) map.set(s.cohortId, PALETTE[i++ % PALETTE.length])
    return map
  }, [sessions])
  ...
  const cohortsInView = [...new Set(sessions.filter((s) => { const d = new Date(s.start); return d.getFullYear() === cursor.y && d.getMonth() === cursor.m }).map((s) => s.cohortId))]
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="overflow-hidden rounded-card border border-line bg-white">
        ...
        <div className="grid grid-cols-7">
          {cells.map((d, i) => {
            ...
            return (
              <div key={i} className="min-h-[88px] border-b border-r border-line-light p-1.5 last:border-r-0 [&:nth-child(7n)]:border-r-0">
                {d != null && (
                  <>
      ...
      {cohortsInView.length > 0 && (
        <div className="flex flex-wrap gap-3">
          {cohortsInView.map((id) => {
            const name = sessions.find((s) => s.cohortId === id)?.cohortName ?? 'Cohort'
            return (
              <span key={id} className="inline-flex items-center gap-1.5 text-[12.5px] text-content-secondary">
                <span className="h-2.5 w-2.5 rounded-full" style={{ background: colorByCohort.get(id) }} /> {name}
              </span>
            )
          })}
        </div>
