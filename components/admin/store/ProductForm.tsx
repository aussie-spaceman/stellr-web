  ...
  return (
    <div className="max-w-md rounded-xl border border-gray-200 bg-white p-5">
      ...
      <select
        ...
        className="mb-4 w-full rounded-md border border-gray-200 px-3 py-2 text-sm capitalize"
      >
        {TYPES.map((t) => (
      ...
      <button
        ...
        className="flex items-center gap-1 rounded-md bg-indigo-600 px-4 py-2 text-sm text-white hover:bg-indigo-700 disabled:opacity-50"
      >
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-xl border border-gray-200 bg-white p-5">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <div>
            ...
            <select
              ...
              className="w-full rounded-md border border-gray-200 px-3 py-2 text-sm capitalize"
            >
              {TYPES.map((t) => (
                <option key={t} value={t}>{t}</option>
              ))}
        ...
        <button
          onClick={saveDetails}
          className="mt-4 flex items-center gap-1 rounded-md bg-indigo-600 px-4 py-2 text-sm text-white hover:bg-indigo-700"
        >
...
function VariantsSection({
  ...
  return (
    <section className="rounded-xl border border-gray-200 bg-white p-5">
      <h2 className="mb-3 text-sm font-semibold text-gray-900">Variants</h2>
      <div className="space-y-2">
        {variants.map((v) => (
          <div key={v.id} className="flex flex-wrap items-center gap-2">
            ...
            {v.pod_sync_variant_id && <span className="text-[11px] text-indigo-500">Printful</span>}
            <button
            ...
            <button onClick={() => removeVariant(v.id)} className="text-gray-400 hover:text-red-600" aria-label="Delete variant">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2 border-t border-gray-100 pt-4">
        <input value={newSku} onChange={(e) => setNewSku(e.target.value)} placeholder="New SKU" className="w-40 rounded-md border border-gray-200 px-2 py-1 text-sm" />
        <input value={newLabel} onChange={(e) => setNewLabel(e.target.value)} placeholder="Label" className="w-40 rounded-md border border-gray-200 px-2 py-1 text-sm" />
        <div className="flex items-center gap-1">
          <span className="text-sm text-gray-400">$</span>
          <input value={newPrice} onChange={(e) => setNewPrice(e.target.value)} placeholder="0.00" inputMode="decimal" className="w-24 rounded-md border border-gray-200 px-2 py-1 text-sm" />
        </div>
        <button
          onClick={addVariant}
...
function PrintfulSection({
  ...
  if (!enabled) {
    return (
      <section className="rounded-xl border border-gray-200 bg-white p-5">
        <h2 className="mb-1 text-sm font-semibold text-gray-900">Printful</h2>
        <p className="text-sm text-gray-400">Printful is not configured in this environment.</p>
      </section>
    )
  }
