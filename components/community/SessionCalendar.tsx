  ...
  return (
    <div>
      <div className="mb-3 flex items-center justify-between">
        <button onClick={() => setOffset(offset - 1)} className="rounded p-1 hover:bg-gray-100">
      ...
      <div className="grid grid-cols-7 gap-px text-center text-xs">
        ...
        {cells.map((day, i) => {
          ...
          return (
            <div
              key={i}
              className={`relative min-h-[2.5rem] rounded p-1 ${isToday ? 'bg-indigo-50' : ''}`}
              title={daySessionList?.map((s) => s.title ?? 'Session').join(', ')}
            >
              {day && (
                <>
                  <span className={`text-xs ${isToday ? 'font-bold text-indigo-600' : 'text-gray-600'}`}>{day}</span>
                  {daySessionList && (
                      ...
                      {daySessionList.slice(0, 3).map((s) => (
                        <span
                          ...
                          className={`inline-block h-1.5 w-1.5 rounded-full ${
                            s.status === 'completed' ? 'bg-green-400' : s.status === 'cancelled' ? 'bg-gray-300' : 'bg-indigo-500'
                          }`}
                        />
                      ))}
                    </div>
