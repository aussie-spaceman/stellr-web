...
export function WorkshopDiscoverGrid({ workshops, creditsRemaining }: { workshops: DiscoverWorkshop[]; creditsRemaining: number }) {
  const [active, setActive] = useState<DiscoverWorkshop | null>(null)

  if (workshops.length === 0) {
    return (
      <div className="rounded-[14px] border border-dashed border-line bg-white py-12 text-center text-sm text-content-muted">
        No open workshops right now. Check back soon.
      </div>
    )
  }
    ...
    <>
      <div className="grid gap-4 sm:grid-cols-2">
        {workshops.map((w) => {
          ...
          const label = accessLabel(w.access.kind, { priceCents: w.access.priceCents, creditCost: w.access.creditCost })
          return (
            <div key={w.id} className="flex flex-col rounded-card border border-line bg-white p-5">
              ...
              <h3 className="mt-3 font-display text-[19px] font-bold text-ink">{w.name}</h3>
              <p className="mt-0.5 text-[13.5px] text-content-secondary">
                {w.mentorName ?? 'Stellr coach'} · starts {fmtStart(w.startDate)} · {w.plannedSessions} session{w.plannedSessions === 1 ? '' : 's'}
              </p>
              {w.blurb && <p className="mt-2 line-clamp-2 text-[13.5px] text-content-muted">{w.blurb}</p>}

              <div className="mt-auto flex items-center justify-between gap-3 pt-4">
                <div>
                  <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-content-faint">Access</p>
                  <p className={`text-sm font-bold ${label.className}`}>{workshopAccessText(w, label.text)}</p>
                </div>
                <button
                  onClick={() => setActive(w)}
      ...
      {active && <RegisterModal workshop={active} creditsRemaining={creditsRemaining} onClose={() => setActive(null)} />}
    </>
  )
}

// accessLabel says "mentoring credit"; re-word for workshop credits.
function workshopAccessText(w: DiscoverWorkshop, fallback: string): string {
  if (w.access.kind === 'credit') {
    const n = w.access.creditCost
    return `${n} workshop credit${n === 1 ? '' : 's'}`
  }
  return fallback
}
...
function RegisterModal({
  ...
  const isFree = workshop.access.kind === 'free'
  const canCredit = workshop.access.canUseCredit && workshop.access.kind !== 'free'
  const hasPaid = workshop.access.priceCents != null && workshop.access.priceCents > 0
  ...
  const submit = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/community/workshops/register', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ workshopId: workshop.id, method: chosen }),
      })
      ...
      router.push(`/community/workshops/${workshop.id}`)
    } catch {
    ...
    >
      ...
      >
        <div className="flex items-start justify-between">
          <h2 className="font-display text-[20px] font-bold text-ink">Register for this workshop</h2>
        ...
        <div className="mt-4 flex items-center gap-3 rounded-[12px] bg-surface p-3.5">
          ...
          <div>
            <p className="font-display text-[16px] font-bold text-ink">{workshop.name}</p>
            <p className="text-[13px] text-content-secondary">
              {workshop.mentorName ?? 'Stellr coach'} · {workshop.plannedSessions} session{workshop.plannedSessions === 1 ? '' : 's'}
            </p>
          </div>
        </div>
          ...
          <div className="mt-4 space-y-2">
            ...
            {hasPaid && (
              <PayOption
                ...
                title={`Pay ${formatUsd(workshop.access.priceCents ?? 0)} one-off`}
                sub="Secure payment via Stripe"
              />
            )}
            {!canCredit && !hasPaid && (
              <p className="rounded-[10px] bg-surface px-4 py-3 text-sm text-content-muted">
                You have no workshop credits left and this workshop has no one-off option. Upgrade your membership to join.
              </p>
            )}
          </div>
        )}
...
function PayOption({ selected, onSelect, title, sub }: { selected: boolean; onSelect: () => void; title: string; sub: string }) {
  return (
    <button
      onClick={onSelect}
