...
- [x] **No volunteer template is needed.** Volunteer is an *event role*; for legal purposes those people execute the **mentor** agreement (confirmed by Stellr, 9 Sept 2026). `createVolunteerAgreementEnvelope()` issues `DOCUSIGN_MENTOR_TEMPLATE_ID` with the `Mentor` role, while still recording `envelope_type = 'volunteer'` so the volunteer programme's own status panel keeps working. `DOCUSIGN_VOLUNTEER_TEMPLATE_ID` no longer exists — do not reintroduce it.
