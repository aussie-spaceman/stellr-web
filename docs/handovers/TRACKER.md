...
| 7.6 | Local `.env.local` carries a **live** Stripe key | Found at close-out (18 Sept): the main checkout's `.env.local` has `STRIPE_SECRET_KEY=sk_live_…` while Supabase/Clerk point at dev. `scripts/production-guard.mjs` checks Supabase/Clerk only, so `npm run dev` starts, and a local checkout would create a **real** Stripe session. Worktrees copy this file. | Swap to the `sk_test` key in `.env.local`; extend `production-guard.mjs` to refuse `sk_live` / `pk_live` unless `APP_ENV=prod`. | ☐ |
...
| 7.8 | CLAUDE.md icon rule changed by inference | The rule now blesses `lucide-react` for UI icons (177 files) and reserves `@stellr/icons` for the brand set. Flagged twice in-session as an assumption; never explicitly confirmed. Live on production via #110. | Maintainer: confirm, or replace with a `ds-lint` rule that enforces `@stellr/icons` and a migration plan for the 177 files. | ☐ |
