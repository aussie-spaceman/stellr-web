...
| 4.7 | Verify the first Production build on the dev project | Pending this PR's merge to `dev`. | Session: `target: production` on the new deployment; cron guard answers as `dev`; `/` does not redirect off the host. Tick here with the deployment id. | ☐ |
