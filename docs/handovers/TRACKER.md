...
| 7.6 | Local `.env.local` carries a **live** Stripe key | **Guard closed 18 Sept** (this PR): `production-guard.mjs` now refuses `STRIPE_SECRET_KEY=sk_live_…` and `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY=pk_live_…`, with tests; verified it trips on the main checkout's current file. **Key swap is the maintainer's** — the local dev server (and local Playwright) will refuse to start until `STRIPE_SECRET_KEY` in `.env.local` is the `sk_test_` key (Stripe → Developers → API keys → Test mode). | Maintainer: swap the key, then `npm run dev` starts again. | ☑ guard / ☐ swap |
