...
| 9.10 | Sandbox guard fired on the dev deployment | **Found and fixed 22 Sept.** `stellr-web-dev` builds `dev` as ITS production, so `VERCEL_ENV=production` and `assertLiveCredentials` threw for every integration there — a 503 on the Checkr order route (proved as admin against the dev deployment), with the same trap under Stripe checkout, Clerk provisioning and DocuSign issuance since 15 Sept. `isRealProductionApp()` now also requires the deployment NOT to declare `NEXT_PUBLIC_APP_ENV=dev`; unset or misspelt still counts as production, so a forgotten variable cannot disarm it. Health endpoint uses the same notion so `problems` means "would actually be refused". | Verify on the dev deployment after this lands: order route no longer 503s. | ☐ |
