...
| 10.7 | Local `.env.local` files lack `NEXT_PUBLIC_MEDIA_BASE_URL` | Found during close-out: the plan said to set it in every checkout and only the Vercel projects and CI were done. Without it `npm run dev` renders relative `/videos/…` paths that now 404. **Fixed in the main checkout 22 Sept**; `../stellr-web-vitest` and `.claude/worktrees/credentials-linkedin` have no `.env.local` at all (pre-existing). | Any new worktree copying `.env.local` inherits it; a worktree created from `.env.local.example` gets the value from there. Nothing further unless a checkout shows missing media. | ☑ (main checkout) |
| 10.8 | Original (pre-re-encode) master MP4s not archived outside git | The plan said keep the 453 MB of originals in Drive; they were left in git history at `9fcabb8` only, and the scratchpad copies die with the session. Recoverable (`git show 9fcabb8:public/videos/<name>.mp4 > out.mp4`) but coupled to never rewriting that history — which item 10.9 would do. | Before any history rewrite, extract the 12 masters from `9fcabb8` and put them in Drive. | ☐ |
| 10.9 | Repo pack is ~1 GB (MP4 history) | Every clone and CI checkout pays for it. Git LFS or a history rewrite; blocked on 10.8. | Maintainer decision; not urgent. | ☐ |
