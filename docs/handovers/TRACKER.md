...
| 4.2 | Sign-out test 10× locally | #79's `waitForURL` did **not** fix it: on #81's run it timed out and the failure snapshot showed `/account` fully signed in as Ada — `window.Clerk?.signOut()` was a silent no-op when Clerk's script had not loaded, and on a dev instance cleared cookies are re-minted from localStorage. #82 uses `clerk.signOut({ page })` from `@clerk/testing` and asserts `/sign-in` positively. CI ran it once, green. | After 4.1: `npx playwright test e2e/core/member-account.spec.ts --repeat-each=10 -g "signing out"`. | ☐ |
